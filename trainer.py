...
class StepByStep(object):
    ...
    def _mini_batch(self, validation=False):
        ...
        for i, (x_batch, y_batch) in enumerate(mini_batch_progress_bar):
            ...
            lr = self.scheduler.get_last_lr() if self.scheduler else 0.0
            mini_batch_progress_bar.set_description(
                f'Minibatch losses: Generator: {loss_generator:.4f}. Discriminator fake: {loss_discriminator_fake:.4f}. Discriminator real: {loss_discriminator_real:.4f}. lr: {lr:.4f}'
            )
    ...
    def train(self, n_epochs, seed=42, n_clip=5):
        ...
        self.set_seed(seed)
        self.scheduler = torch.optim.lr_scheduler.LinearLR(self.discriminator_optimizer, start_factor=0.1, total_iters=n_epochs*len(self.train_loader))
        self.n_clip = n_clip
